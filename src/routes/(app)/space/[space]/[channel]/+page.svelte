<script lang="ts">
  ...
  $effect(() => {
    if (!space || !channel || !showVideoChat || !videoRef) {
      return;
    }

    // Initialize videoSignaling if needed
    if (!channel.videoSignaling) {
      space.change((doc) => {
    ...
    Object.entries(channel.videoSignaling).forEach(([peerId, messages]) => {
      // Only process the latest message from each peer
      const latestMessage = messages[messages.length - 1];
      if (latestMessage) {
        videoRef?.handleSignalingMessage(peerId, latestMessage);
      }
    });
  });
  ...
  function sendSignalingMessage(targetPeerId: string, message: any) {
    if (!space || !user.agent || !page.params.channel) return;
    
    space.change((doc) => {
      const channel = doc.channels[page.params.channel];
      if (!channel) return;

      // Initialize videoSignaling if needed
      if (!channel.videoSignaling) {
        channel.videoSignaling = {};
      }
      if (!channel.videoSignaling[targetPeerId]) {
        channel.videoSignaling[targetPeerId] = [];
      }

      // Keep only the latest message of each type
      const messages = channel.videoSignaling[targetPeerId];
      const idx = messages.findIndex(m => m.type === message.type);
      if (idx !== -1) {
