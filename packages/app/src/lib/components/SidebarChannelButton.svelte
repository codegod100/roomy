...
{#if channel && !channel.softDeleted}
  ...
  <div class="group flex items-center gap-1 relative">
    ...
    >
      <Icon icon={channel.channelType === "feeds" ? "basil:feed-outline" : channel.channelType === "links" ? "basil:link-outline" : "basil:comment-solid"} class="shrink-0" />
      ...
      {:else if channel.channelType === "links"}
        <span class="text-xs bg-secondary/20 text-secondary px-1 py-0.5 rounded shrink-0">LINKS</span>
      {/if}
